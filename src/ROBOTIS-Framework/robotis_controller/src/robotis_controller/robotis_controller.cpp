...
// =============================================================================
// Part of robotis_controller package
// =============================================================================
 ...
 *      Author: zerom
 * 
 * RobotisController adalah kelas utama yang mengelola:
 * - Komunikasi dengan motor Dynamixel melalui protokol serial
 * - Koordinasi antar motion modules (walking, action, head, dll)
 * - Sinkronisasi baca/tulis data ke multiple motor (bulk read/sync write)
 * - Timer loop untuk kontrol real-time
 */

// Menyertakan library YAML untuk parsing file konfigurasi
#include <yaml-cpp/yaml.h>
// Menyertakan header file kelas RobotisController
...
// Menggunakan namespace robotis_framework
using namespace robotis_framework;

// Konstruktor RobotisController - Singleton pattern
// Menginisialisasi controller utama robot ROBOTIS
RobotisController::RobotisController()
  : Node("robotis_controller"),           // Inisialisasi node ROS2
    is_timer_running_(false),              // Timer belum berjalan
    is_offset_enabled_(true),              // Offset joint diaktifkan
    offset_ratio_(1.0),                    // Rasio offset (1.0 = full offset)
    stop_timer_(false),                    // Flag stop timer
    init_pose_loaded_(false),              // Pose awal belum dimuat
    timer_thread_(0),                      // Thread timer belum dibuat
    controller_mode_(MotionModuleMode),    // Mode: menggunakan motion module
    DEBUG_PRINT(false),                    // Mode debug nonaktif
    robot_(0),                             // Pointer ke objek Robot (belum diinisialisasi)
    gazebo_mode_(false),                   // Mode Gazebo simulator nonaktif
    gazebo_robot_name_("robotis")          // Nama robot di Gazebo
{
  // Membersihkan map direct sync write
  direct_sync_write_.clear();
}

// Fungsi untuk menginisialisasi sync write ke semua Dynamixel
// Sync write memungkinkan pengiriman data ke multiple motor dalam satu paket
// Ini meningkatkan efisiensi komunikasi dan sinkronisasi gerakan
void RobotisController::initializeSyncWrite()
{
  // Jika mode Gazebo aktif, tidak perlu komunikasi hardware
  if (gazebo_mode_ == true)
    return;

  // Melakukan bulk read pertama untuk mendapatkan posisi awal semua motor
  RCLCPP_INFO(this->get_logger(), "FIRST BULKREAD");
  // Mengirim paket bulk read ke semua port
  for (auto& it : port_to_bulk_read_)
    it.second->txRxPacket();
  
  // Retry bulk read jika gagal (maksimal 10 kali)
  for(auto& it : port_to_bulk_read_)
  {
    int error_count = 0;
    ...
    do
    {
      // Jika gagal lebih dari 10 kali, keluar dengan error
      if (++error_count > 10)
      {
        RCLCPP_ERROR(this->get_logger(), "first bulk read fail!!");
        exit(-1);
      }
      // Tunggu 10ms sebelum retry
      usleep(10 * 1000);
      result = it.second->txRxPacket();
    } while (result != COMM_SUCCESS);
  }
  // Menandai bahwa pose awal sudah berhasil dimuat

...
// ROBOTIS-OP3/op3_kinematics_dynamics/src/op3_kinematics_dynamics.cpp

// Menyertakan library iostream untuk operasi input/output
#include <iostream>
// Menyertakan header file kelas OP3KinematicsDynamics
...
// Fungsi helper untuk mendapatkan tanda (sign) dari sebuah angka
// Mengembalikan -1.0 jika negatif, 1.0 jika positif atau nol
double getSign(double num)
{
  if(num < 0)
    return -1.0;  // Angka negatif
  else
    return 1.0;   // Angka positif atau nol
}

// Konstruktor default kelas OP3KinematicsDynamics
OP3KinematicsDynamics::OP3KinematicsDynamics()
{
}

// Destruktor kelas OP3KinematicsDynamics
OP3KinematicsDynamics::~OP3KinematicsDynamics()
{
}

// Konstruktor dengan parameter TreeSelect untuk memilih model kinematik
// Parameter tree menentukan bagian robot yang akan dimodelkan (WholeBody = seluruh tubuh)
OP3KinematicsDynamics::OP3KinematicsDynamics(TreeSelect tree)
{
  // Mengalokasikan memori untuk semua link data (dari ID 0 sampai ALL_JOINT_ID)
  for (int id = 0; id <= ALL_JOINT_ID; id++)
    op3_link_data_[id] = new LinkData();

  // Jika model yang dipilih adalah WholeBody (seluruh tubuh)
  if (tree == WholeBody)
  {
    // Konfigurasi link base (root/dasar robot)
...
// Fungsi untuk mencari rute (path) dari base ke joint tertentu
// Menggunakan rekursi untuk menelusuri kinematic tree ke atas
// Parameter: to = ID joint tujuan
// Return: vector berisi ID joint dari base ke tujuan
std::vector<int> OP3KinematicsDynamics::findRoute(int to)
{
  // Mendapatkan ID parent dari joint tujuan
  int id = op3_link_data_[to]->parent_;

  // Vector untuk menyimpan rute
  std::vector<int> idx;

  // Jika parent adalah base (ID 0), ini adalah base case rekursi
  if (id == 0)
  {
    idx.push_back(0);     // Tambahkan base
    idx.push_back(to);    // Tambahkan tujuan
  }
  else
  {
    // Rekursi: cari rute ke parent terlebih dahulu
    idx = findRoute(id);
    // Tambahkan joint tujuan di akhir
    idx.push_back(to);
  }
...
// Fungsi untuk mencari rute dari joint tertentu ke joint lain
// Parameter: from = ID joint asal, to = ID joint tujuan
// Return: vector berisi ID joint dari asal ke tujuan
std::vector<int> OP3KinematicsDynamics::findRoute(int from, int to)
{
  ...
  // Base case: jika parent adalah joint asal
  if (id == from)
  {
    idx.push_back(from);  // Tambahkan asal
    idx.push_back(to);    // Tambahkan tujuan
  }
  else if (id != 0)
  {
    // Rekursi: cari rute dari asal ke parent
...
// Fungsi rekursif untuk menghitung total massa robot dari joint tertentu
// Menjumlahkan massa link saat ini dengan massa sibling dan child
// Parameter: joint_id = ID joint yang akan dihitung massanya
// Return: total massa dalam kg
double OP3KinematicsDynamics::calcTotalMass(int joint_id)
{
  double mass;

  // Base case: jika joint_id invalid (-1), massa = 0
  if (joint_id == -1)
    mass = 0.0;
  else
    // Rekursi: massa link ini + massa sibling + massa child
...
// Fungsi rekursif untuk menghitung massa * posisi center of mass (MC)
// Digunakan untuk perhitungan Center of Mass (COM) seluruh robot
// Parameter: joint_id = ID joint yang akan dihitung
// Return: vektor 3x1 berisi mc (massa * posisi CoM dalam world frame)
Eigen::MatrixXd OP3KinematicsDynamics::calcMC(int joint_id)
{
  // Vektor untuk menyimpan hasil mc
  Eigen::MatrixXd mc(3, 1);

  // Base case: jika joint_id invalid (-1), mc = 0
  if (joint_id == -1)
    mc = Eigen::MatrixXd::Zero(3, 1);
  else
  {
    // Menghitung mc untuk link ini:
    // mc = massa * (rotasi * CoM_lokal + posisi_link)
    ...
    // Rekursi: tambahkan mc dari sibling dan child
...
// Fungsi untuk menghitung Center of Mass (COM) seluruh robot
// COM = total(massa * posisi) / total_massa
// Parameter: mc = hasil dari calcMC() 
// Return: vektor 3x1 berisi posisi COM dalam world frame
Eigen::MatrixXd OP3KinematicsDynamics::calcCOM(Eigen::MatrixXd mc)
{
  double mass;
  Eigen::MatrixXd COM(3, 1);

  // Menghitung total massa robot dari base (ID 0)
...
// Fungsi rekursif untuk menghitung Forward Kinematics
// Menghitung posisi dan orientasi setiap link berdasarkan sudut joint
// Parameter: joint_id = ID joint yang akan dihitung (mulai dari 0 untuk seluruh robot)
void OP3KinematicsDynamics::calcForwardKinematics(int joint_id)
{
  // Base case: jika joint_id invalid, return
  if (joint_id == -1)
    return;

  // Jika joint adalah base (ID 0)
  if (joint_id == 0)
  {
    // Posisi base di origin (0, 0, 0)
    op3_link_data_[0]->position_ = Eigen::MatrixXd::Zero(3, 1);
    // Orientasi base dihitung dari sudut joint menggunakan formula Rodrigues
  ...
  // Jika bukan base, hitung posisi dan orientasi dari parent
  if (joint_id != 0)
  {
    // Mendapatkan ID parent
    int parent = op3_link_data_[joint_id]->parent_;

    // Posisi link = rotasi_parent * posisi_relatif + posisi_parent
    ...
    // Orientasi link = orientasi_parent * rotasi_joint (menggunakan Rodrigues)
...
// Fungsi untuk menghitung error posisi dan orientasi (velocity-level error)
// Digunakan dalam iterasi Inverse Kinematics
// Parameter: tar_position = posisi target, curr_position = posisi saat ini
//            tar_orientation = orientasi target, curr_orientation = orientasi saat ini
// Return: vektor error 6x1 (3 posisi + 3 orientasi)
...
{
  // Error posisi = posisi_target - posisi_sekarang
  Eigen::MatrixXd pos_err = tar_position - curr_position;
  // Error orientasi dalam frame lokal
  Eigen::MatrixXd ori_err = curr_orientation.transpose() * tar_orientation;
  // Konversi error orientasi ke vektor angular velocity dalam world frame
  Eigen::MatrixXd ori_err_dash = curr_orientation * robotis_framework::convertRotToOmega(ori_err);

  // Gabungkan error posisi dan orientasi menjadi vektor 6x1
  Eigen::MatrixXd err = Eigen::MatrixXd::Zero(6, 1);
  err.block<3, 1>(0, 0) = pos_err;      // Error posisi (x, y, z)
  err.block<3, 1>(3, 0) = ori_err_dash; // Error orientasi (roll, pitch, yaw rate)

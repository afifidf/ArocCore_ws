...
// Menyertakan header file kelas OpenCRModule
#include "open_cr_module/open_cr_module.h"

// Menggunakan namespace robotis_op
using namespace robotis_op;

// Konstruktor kelas OpenCRModule - modul untuk membaca sensor dari OpenCR board
// OpenCR adalah sub-controller yang menangani IMU, tombol, dan power management
OpenCRModule::OpenCRModule()
    : Node("open_cr_module"),         // Inisialisasi node ROS2
      control_cycle_msec_(8),          // Siklus kontrol 8ms (125 Hz)
      DEBUG_PRINT(false),              // Mode debug nonaktif
      present_volt_(0.0)               // Tegangan saat ini (Volt)
{
  // Nama modul untuk identifikasi
  module_name_ = "open_cr_module";

  // Inisialisasi hasil pembacaan gyroscope (dalam rad/s)
  result_["gyro_x"] = 0.0;  // Gyro sumbu X
  result_["gyro_y"] = 0.0;  // Gyro sumbu Y
  result_["gyro_z"] = 0.0;  // Gyro sumbu Z

  // Inisialisasi hasil pembacaan accelerometer (dalam m/s^2)
  result_["acc_x"] = 0.0;   // Akselerasi sumbu X
  result_["acc_y"] = 0.0;   // Akselerasi sumbu Y
  result_["acc_z"] = 0.0;   // Akselerasi sumbu Z

  // Inisialisasi status tombol (0 = tidak ditekan)
  result_["button_mode"] = 0;   // Tombol Mode
  result_["button_start"] = 0;  // Tombol Start
  result_["button_user"] = 0;   // Tombol User

  // Inisialisasi tegangan baterai
  result_["present_voltage"] = 0.0;

  // Status tombol (untuk deteksi perubahan state)
  buttons_["button_mode"] = false;      // Status tombol Mode
  buttons_["button_start"] = false;     // Status tombol Start
  buttons_["button_user"] = false;      // Status tombol User
  buttons_["published_mode"] = false;   // Flag: sudah publish event Mode
  buttons_["published_start"] = false;  // Flag: sudah publish event Start
  buttons_["published_user"] = false;   // Flag: sudah publish event User

  // Hasil pembacaan sebelumnya untuk low-pass filter gyroscope
  previous_result_["gyro_x"] = 0.0;
  previous_result_["gyro_y"] = 0.0;
  previous_result_["gyro_z"] = 0.0;

  // Hasil pembacaan 2 langkah sebelumnya (untuk filter)
  previous_result_["gyro_x_prev"] = 0.0;
  previous_result_["gyro_y_prev"] = 0.0;
  previous_result_["gyro_z_prev"] = 0.0;

  // Hasil pembacaan sebelumnya untuk accelerometer
  previous_result_["acc_x"] = 0.0;
  previous_result_["acc_y"] = 0.0;
  previous_result_["acc_z"] = 0.0;

  // Menyimpan waktu pesan terakhir
...
// Thread untuk menangani callback ROS dan publishing

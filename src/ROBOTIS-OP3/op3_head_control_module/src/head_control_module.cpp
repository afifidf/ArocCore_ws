...
// =============================================================================
// Head Control Module untuk Robot OP3
// - Mengontrol gerakan kepala (pan & tilt)
// - Tracking objek (bola, wajah)
// - Scan mode untuk mencari objek
// =============================================================================
...
// Menyertakan library C standard untuk I/O
#include <stdio.h>
// Menyertakan header file kelas HeadControlModule
...
// Konstruktor kelas HeadControlModule - menginisialisasi modul kontrol kepala
HeadControlModule::HeadControlModule()
  : Node("op3_head_control_module"),   // Inisialisasi node ROS2
    control_cycle_msec_(0),             // Siklus kontrol (akan diset saat initialize)
    stop_process_(false),               // Flag untuk menghentikan proses
    is_moving_(false),                  // Status: kepala sedang bergerak
    is_direct_control_(true),           // Mode: kontrol langsung (bukan trajectory)
    tra_count_(0),                      // Counter langkah trajectory
    tra_size_(0),                       // Total langkah trajectory
    moving_time_(3.0),                  // Waktu default untuk pergerakan (3 detik)
    scan_state_(NoScan),                // Status scan: tidak scanning
    has_goal_position_(false),          // Flag: sudah memiliki goal position
    angle_unit_(35),                    // Unit sudut untuk scanning (35 derajat)
    DEBUG(false)                        // Mode debug nonaktif
{
  ...
  // Inisialisasi result untuk joint kepala
  result_["head_pan"] = new robotis_framework::DynamixelState();   // Joint pan (kiri-kanan)
  result_["head_tilt"] = new robotis_framework::DynamixelState();  // Joint tilt (atas-bawah)

  // Mapping nama joint ke indeks
  using_joint_name_["head_pan"] = 0;   // Pan = indeks 0
  using_joint_name_["head_tilt"] = 1;  // Tilt = indeks 1

  // Batas sudut maksimum dan minimum untuk setiap joint
  max_angle_[using_joint_name_["head_pan"]] = 85 * DEGREE2RADIAN;   // Pan max: 85 derajat ke kanan
  min_angle_[using_joint_name_["head_pan"]] = -85 * DEGREE2RADIAN;  // Pan min: 85 derajat ke kiri
  max_angle_[using_joint_name_["head_tilt"]] = 30 * DEGREE2RADIAN;  // Tilt max: 30 derajat ke atas
  min_angle_[using_joint_name_["head_tilt"]] = -75 * DEGREE2RADIAN; // Tilt min: 75 derajat ke bawah

  // Inisialisasi matriks untuk menyimpan posisi, kecepatan, dan akselerasi
  target_position_ = Eigen::MatrixXd::Zero(1, result_.size());      // Posisi target
  current_position_ = Eigen::MatrixXd::Zero(1, result_.size());     // Posisi saat ini
  goal_position_ = Eigen::MatrixXd::Zero(1, result_.size());        // Posisi goal
  goal_velocity_ = Eigen::MatrixXd::Zero(1, result_.size());        // Kecepatan goal
  goal_acceleration_ = Eigen::MatrixXd::Zero(1, result_.size());    // Akselerasi goal

  // Menyimpan waktu pesan terakhir diterima
...
{
  // Mendeklarasikan dan mengambil parameter angle_unit dari ROS
  this->declare_parameter("angle_unit", 35.0);
  angle_unit_ = this->get_parameter("angle_unit").as_double();

  // Menampilkan info angle unit yang digunakan
...
// Thread untuk menangani callback ROS
void HeadControlModule::queueThread()
{
  // Membuat executor single-threaded
  auto executor = rclcpp::executors::SingleThreadedExecutor();
  executor.add_node(this->get_node_base_interface());

  // Subscriber untuk menerima perintah posisi joint kepala (absolut)
  ...
  // Subscriber untuk menerima perintah posisi joint kepala (offset/relatif)
  ...
  // Subscriber untuk menerima perintah scan (mencari objek)
  ...
  // Loop utama untuk memproses callback
...
// Callback untuk menerima perintah posisi joint (absolut)
void HeadControlModule::setHeadJointCallback(const sensor_msgs::msg::JointState::SharedPtr msg)
{
  // Memanggil setHeadJoint dengan mode non-offset (absolut)
  setHeadJoint(msg, false);
}

// Callback untuk menerima perintah posisi joint (offset/relatif)
void HeadControlModule::setHeadJointOffsetCallback(const sensor_msgs::msg::JointState::SharedPtr msg)
{
  // Memanggil setHeadJoint dengan mode offset (relatif dari posisi saat ini)

...
// ROBOTIS-OP3/op3_direct_control_module/src/direct_control_module.cpp

// Menyertakan library C standard untuk I/O
#include <cstdio>
// Menyertakan header file kelas DirectControlModule
#include "op3_direct_control_module/direct_control_module.h"

// Menggunakan chrono literals untuk kemudahan penulisan durasi waktu
...
// Konstruktor kelas DirectControlModule - modul untuk kontrol langsung joint
// Memungkinkan pengiriman posisi joint secara langsung tanpa melalui trajectory planner
DirectControlModule::DirectControlModule()
  : Node("op3_direct_control_module"),   // Inisialisasi node ROS2
    control_cycle_msec_(0),               // Siklus kontrol (diset saat initialize)
    stop_process_(false),                 // Flag untuk menghentikan proses
    is_moving_(false),                    // Status: sedang bergerak
    is_updated_(false),                   // Flag: posisi sudah diupdate
    is_blocked_(false),                   // Flag: gerakan diblokir (collision)
    r_min_diff_(0.07),                    // Jarak minimum untuk deteksi collision tangan kanan
    l_min_diff_(0.07),                    // Jarak minimum untuk deteksi collision tangan kiri
    tra_count_(0),                        // Counter langkah trajectory
    tra_size_(0),                         // Total langkah trajectory
    default_moving_time_(0.5),            // Waktu default pergerakan (0.5 detik)
    default_moving_angle_(30.0),          // Sudut default pergerakan (30 derajat)
    check_collision_(true),               // Flag: periksa collision
    moving_time_(3.0),                    // Waktu pergerakan saat ini
    BASE_INDEX(0),                        // Indeks link base dalam kinematik
    HEAD_INDEX(20),                       // Indeks link kepala
    RIGHT_END_EFFECTOR_INDEX(21),         // Indeks end effector tangan kanan
    RIGHT_ELBOW_INDEX(5),                 // Indeks siku kanan
    LEFT_END_EFFECTOR_INDEX(22),          // Indeks end effector tangan kiri
    LEFT_ELBOW_INDEX(6),                  // Indeks siku kiri
...
// Fungsi inisialisasi modul - dipanggil saat modul dimuat ke controller
void DirectControlModule::initialize(const int control_cycle_msec, robotis_framework::Robot *robot)
{
  ...
  // Digunakan untuk perhitungan collision detection
  op3_kinematics_ = new OP3KinematicsDynamics(WholeBody);

  // Inisialisasi result dan tabel ID joint
  int joint_index = 0;
       ...
       it != robot->dxls_.end(); it++)
  {
    ...
    // Membuat objek DynamixelState untuk setiap joint
    result_[joint_name] = new robotis_framework::DynamixelState();
    // Menyalin goal position dari state Dynamixel
    result_[joint_name]->goal_position_ = dxl_info->dxl_state_->goal_position_;

    // Inisialisasi status collision untuk setiap joint (tidak ada collision)
  ...
  present_position_ = Eigen::MatrixXd::Zero(1, result_.size());     // Posisi saat ini
  ...
  // Mendeklarasikan dan mengambil parameter dari ROS

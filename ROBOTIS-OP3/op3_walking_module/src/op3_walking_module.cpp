...
// =============================================================================
// Walking Module untuk Robot OP3
// - Mengontrol gerakan berjalan robot bipedal
// - Pattern generator untuk footsteps
// - Balance control menggunakan sensor IMU
// =============================================================================
...
// Menyertakan library untuk mendapatkan path package ROS2
#include <ament_index_cpp/get_package_share_directory.hpp>
// Menyertakan header file kelas WalkingModule
...
// Konstanta fase balancing untuk gait cycle
// DSP = Double Support Phase (kedua kaki menyentuh tanah)
// SSP = Single Support Phase (satu kaki mengangkat)
static const int BalancingPhase0 = 0; // DSP : START - fase awal
static const int BalancingPhase1 = 1; // DSP : R--O->L - transisi ke kaki kiri
static const int BalancingPhase2 = 2; // SSP : L_BALANCING1 - balancing kaki kiri fase 1
static const int BalancingPhase3 = 3; // SSP : L_BALANCING2 - balancing kaki kiri fase 2
static const int BalancingPhase4 = 4; // DSP : R--O<-L - transisi dari kaki kiri
static const int BalancingPhase5 = 5; // DSP : R<-O--L - transisi ke kaki kanan
static const int BalancingPhase6 = 6; // SSP : R_BALANCING1 - balancing kaki kanan fase 1
static const int BalancingPhase7 = 7; // SSP : R_BALANCING2 - balancing kaki kanan fase 2
static const int BalancingPhase8 = 8; // DSP : R->O--L - transisi dari kaki kanan
static const int BalancingPhase9 = 9; // DSP : END - fase akhir

// Konstruktor kelas WalkingModule - menginisialisasi modul berjalan
WalkingModule::WalkingModule()
  : Node("op3_walking_module"),    // Inisialisasi node ROS2 dengan nama "op3_walking_module"
    control_cycle_msec_(8),         // Siklus kontrol 8ms (125 Hz)
    DEBUG(false)                    // Mode debug nonaktif
{
  // Modul belum diaktifkan
  enable_ = false;
  // Nama modul untuk identifikasi
  module_name_ = "walking_module";
  // Mode kontrol menggunakan kontrol posisi
  control_mode_ = robotis_framework::PositionControl;

  // Counter untuk pose inisialisasi
  init_pose_count_ = 0;
  // Status walking: siap tapi belum berjalan
  walking_state_ = WalkingReady;
  // Amplitudo gerakan X sebelumnya (untuk smooth transition)
  previous_x_move_amplitude_ = 0.0;

  // Membuat objek kinematika dan dinamika untuk seluruh tubuh robot
  op3_kd_ = new OP3KinematicsDynamics(WholeBody);

  // Inisialisasi result untuk joint kaki kanan
  // Setiap joint memiliki objek DynamixelState untuk menyimpan goal position
  result_["r_hip_yaw"] = new robotis_framework::DynamixelState();    // Hip yaw kanan (rotasi)
  result_["r_hip_roll"] = new robotis_framework::DynamixelState();   // Hip roll kanan (miring samping)
  result_["r_hip_pitch"] = new robotis_framework::DynamixelState();  // Hip pitch kanan (maju/mundur)
  result_["r_knee"] = new robotis_framework::DynamixelState();       // Lutut kanan
  result_["r_ank_pitch"] = new robotis_framework::DynamixelState();  // Ankle pitch kanan
  result_["r_ank_roll"] = new robotis_framework::DynamixelState();   // Ankle roll kanan

  // Inisialisasi result untuk joint kaki kiri
  result_["l_hip_yaw"] = new robotis_framework::DynamixelState();    // Hip yaw kiri
  result_["l_hip_roll"] = new robotis_framework::DynamixelState();   // Hip roll kiri
  result_["l_hip_pitch"] = new robotis_framework::DynamixelState();  // Hip pitch kiri
  result_["l_knee"] = new robotis_framework::DynamixelState();       // Lutut kiri
  result_["l_ank_pitch"] = new robotis_framework::DynamixelState();  // Ankle pitch kiri
  result_["l_ank_roll"] = new robotis_framework::DynamixelState();   // Ankle roll kiri

  // Inisialisasi result untuk joint bahu (untuk arm swing saat berjalan)
  result_["r_sho_pitch"] = new robotis_framework::DynamixelState();  // Bahu pitch kanan
  result_["l_sho_pitch"] = new robotis_framework::DynamixelState();  // Bahu pitch kiri

  // Tabel mapping nama joint ke indeks array untuk kaki kanan
  ...
  // Tabel mapping nama joint ke indeks array untuk kaki kiri
  ...
  // Tabel mapping untuk joint bahu
  joint_table_["r_sho_pitch"] = 12;
  joint_table_["l_sho_pitch"] = 13;

  // Inisialisasi matriks untuk menyimpan posisi joint
  target_position_ = Eigen::MatrixXd::Zero(1, result_.size());       // Posisi target
  goal_position_ = Eigen::MatrixXd::Zero(1, result_.size());         // Posisi goal (yang dikirim)
  init_position_ = Eigen::MatrixXd::Zero(1, result_.size());         // Posisi awal
  joint_axis_direction_ = Eigen::MatrixXi::Zero(1, result_.size());  // Arah sumbu joint

  // Indeks fase balancing dimulai dari fase 0
  balancing_idx_ = BalancingPhase0;
}

// Destruktor - membersihkan thread
WalkingModule::~WalkingModule()
{
  // Menunggu thread queue selesai sebelum destroy objek
  queue_thread_.join();
}

// Fungsi inisialisasi modul - dipanggil saat modul dimuat
void WalkingModule::initialize(const int control_cycle_msec, robotis_framework::Robot *robot)
{
  // Membuat thread terpisah untuk menangani callback ROS
  queue_thread_ = std::thread(&WalkingModule::queueThread, this);
  // Menyimpan waktu siklus kontrol
  control_cycle_msec_ = control_cycle_msec;

  // ===== Parameter Walking (satuan: meter, detik, radian) =====
  
  // Parameter pose awal (offset dari posisi default)
  walking_param_.init_x_offset = -0.010;              // Offset X (maju/mundur) dalam meter
  walking_param_.init_y_offset = 0.005;               // Offset Y (kiri/kanan) dalam meter
  walking_param_.init_z_offset = 0.020;               // Offset Z (tinggi) dalam meter
  walking_param_.init_roll_offset = 0.0;              // Offset roll (tidak ada)
  walking_param_.init_pitch_offset = 0.0 * DEGREE2RADIAN;   // Offset pitch
  walking_param_.init_yaw_offset = 0.0 * DEGREE2RADIAN;     // Offset yaw
  walking_param_.hip_pitch_offset = 13.0 * DEGREE2RADIAN;   // Offset pitch hip (13 derajat)
  
  // Parameter waktu
  walking_param_.period_time = 600 * 0.001;           // Periode satu langkah (600ms)
  walking_param_.dsp_ratio = 0.1;                     // Rasio double support phase (10%)
  walking_param_.step_fb_ratio = 0.28;                // Rasio step forward/backward
  
  // Parameter gerakan berjalan
  walking_param_.x_move_amplitude = 0.0;              // Amplitudo gerakan X (maju/mundur)
  walking_param_.y_move_amplitude = 0.0;              // Amplitudo gerakan Y (kiri/kanan)
  walking_param_.z_move_amplitude = 0.040;            // Tinggi angkat kaki (4cm)
  walking_param_.angle_move_amplitude = 0.0;          // Amplitudo rotasi (berputar)
  
  // Parameter balance control
  walking_param_.balance_enable = true;               // Aktifkan balance control
  walking_param_.balance_hip_roll_gain = 0.5;         // Gain untuk hip roll
  walking_param_.balance_knee_gain = 0.3;             // Gain untuk lutut
  walking_param_.balance_ankle_roll_gain = 1.0;       // Gain untuk ankle roll
  walking_param_.balance_ankle_pitch_gain = 0.9;      // Gain untuk ankle pitch
  walking_param_.y_swap_amplitude = 0.020;            // Amplitudo swap Y (2cm)
  walking_param_.z_swap_amplitude = 0.005;            // Amplitudo swap Z (0.5cm)
  walking_param_.pelvis_offset = 3.0 * DEGREE2RADIAN; // Offset pelvis (3 derajat)
  walking_param_.arm_swing_gain = 1.5;                // Gain untuk ayunan lengan

  // Variabel untuk body swing
  body_swing_y = 0;  // Swing Y body
  body_swing_z = 0;  // Swing Z body

  // Parameter phase shift untuk pattern generator
  x_swap_phase_shift_ = M_PI;      // Phase shift untuk swap X (180 derajat)
  x_swap_amplitude_shift_ = 0;     // Amplitude shift untuk swap X

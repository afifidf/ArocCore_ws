...
// action_module.cpp
// =============================================================================
// Action Module untuk Robot OP3
// - Menjalankan gerakan pre-defined (motion pages)
// - Membaca motion file (.bin)
// - Interpolasi antar pose untuk gerakan smooth
// =============================================================================
...
// ROBOTIS-OP3/op3_action_module/src/action_module.cpp
...
// Destruktor - membersihkan resource
ActionModule::~ActionModule()
{
  // Menunggu thread queue selesai sebelum destroy objek
  queue_thread_.join();

  // Menutup file motion jika masih terbuka
  if (action_file_ != 0)
    fclose(action_file_);
}
...
void ActionModule::initialize(const int control_cycle_msec, robotis_framework::Robot *robot)
{
  ...
  for (auto& it : robot->dxls_)
  {
    ...
    // Membuat mapping nama joint ke ID dan sebaliknya
    joint_name_to_id_[joint_name] = dxl_info->id_;
    joint_id_to_name_[dxl_info->id_] = joint_name;
    
    // Membuat objek DynamixelState untuk action result
    ...
    // Membuat objek DynamixelState untuk result utama
    ...
    // Semua joint dinonaktifkan untuk action secara default
    action_joints_enable_[joint_name] = false;
  }

  // Path default untuk file motion binary
  std::string path = ament_index_cpp::get_package_share_directory("op3_action_module") + "/data/motion_4095.bin";

  // Mendapatkan path file motion dari parameter ROS
  ...
  // Memuat file motion binary
  loadFile(action_file_path);

  // Memastikan tidak ada playback yang berjalan
  playing_ = false;
}

// Thread untuk menangani callback ROS (subscriber, publisher, service)
void ActionModule::queueThread()
{
  ...
  // Membuat publisher untuk mengirim status modul
  status_msg_pub_ = this->create_publisher<robotis_controller_msgs::msg::StatusMsg>("/robotis/status", 10);
  // Membuat publisher untuk mengirim notifikasi gerakan selesai
  done_msg_pub_ = this->create_publisher<std_msgs::msg::String>("/robotis/movement_done", 10);

  // Membuat subscriber untuk menerima nomor page motion yang akan dimainkan
  ...
  // Membuat subscriber untuk menerima perintah start action dengan konfigurasi
  ...
  // Membuat service server untuk mengecek apakah action sedang berjalan

...
// op3_manager.cpp
// =============================================================================
// Main Controller untuk Robot ROBOTIS OP3
// - Mengelola komunikasi dengan motor Dynamixel
// - Menjalankan motion modules (walking, action, head)
// - Interface antara ROS2 dan hardware robot
// =============================================================================
...
// Menggunakan namespace robotis_framework untuk akses kelas controller
using namespace robotis_framework;
// Menggunakan namespace dynamixel untuk akses SDK Dynamixel
using namespace dynamixel;
// Menggunakan namespace robotis_op untuk akses modul-modul OP3
using namespace robotis_op;

// Konstanta untuk konfigurasi komunikasi Dynamixel
const int BAUD_RATE = 2000000;              // Baud rate default (2 Mbps)
const double PROTOCOL_VERSION = 2.0;         // Versi protokol Dynamixel (2.0)
const int SUB_CONTROLLER_ID = 200;           // ID sub-controller (OpenCR)
const int DXL_BROADCAST_ID = 254;            // ID broadcast untuk semua Dynamixel
const int DEFAULT_DXL_ID = 1;                // ID Dynamixel default untuk testing
const std::string SUB_CONTROLLER_DEVICE = "/dev/ttyUSB0";  // Device port default
const int POWER_CTRL_TABLE = 24;             // Alamat register kontrol power
const int RGB_LED_CTRL_TABLE = 26;           // Alamat register kontrol LED RGB
const int TORQUE_ON_CTRL_TABLE = 64;         // Alamat register torque enable

// Variabel global untuk konfigurasi
bool g_is_simulation = false;                // Flag mode simulasi
int g_baudrate;                              // Baud rate yang digunakan
std::string g_offset_file;                   // Path file offset joint
std::string g_robot_file;                    // Path file konfigurasi robot
std::string g_init_file;                     // Path file inisialisasi
std::string g_device_name;                   // Nama device serial port

// Publisher global untuk mengirim pesan
rclcpp::Publisher<std_msgs::msg::String>::SharedPtr g_init_pose_pub;    // Publisher untuk initial pose
rclcpp::Publisher<std_msgs::msg::String>::SharedPtr g_demo_command_pub; // Publisher untuk perintah demo

// Callback handler untuk tombol pada robot
// Menangani berbagai aksi berdasarkan tombol yang ditekan
void buttonHandlerCallback(const std_msgs::msg::String::SharedPtr msg)
{
  // Jika tombol user ditekan lama (long press)
